...
import { brand, type BlockValueOf, type ListItemValue, type TableCellValue, type TableRowValue } from "../core/values.ts";
import {
  assertNoChildren,
  blocks,
  caption,
...
export type { BlockInput, ListItemInput, TableCellInput, TableRowInput } from "./shared.ts";
...
export function pre(first?: PreOptions | RichTextInput, ...rest: readonly RichTextInput[]) {
  const [options, children] = splitOptions<PreOptions, RichTextInput>(first, rest, "pre()", ["language"], true);
...
export function list(...items: readonly ListItemInput[]): BlockValueOf<"list"> {
  return block({ type: "list", items: listItems(items, "list()") } as Extract<InputRichBlock, { type: "list" }>);
}
...
export function blockQuote(...children: readonly BlockInput[]): BlockValueOf<"blockquote">;
export function blockQuote(options: BlockQuoteOptions, ...children: readonly BlockInput[]): BlockValueOf<"blockquote">;
export function blockQuote(first?: BlockQuoteOptions | BlockInput, ...rest: readonly BlockInput[]) {
  const [options = {}, children] = splitOptions<BlockQuoteOptions, BlockInput>(first, rest, "blockQuote()", ["credit"]);
  return block({
    type: "blockquote",
...
export function pullQuote(first?: PullQuoteOptions | RichTextInput, ...rest: readonly RichTextInput[]) {
  const [options = {}, children] = splitOptions<PullQuoteOptions, RichTextInput>(first, rest, "pullQuote()", ["credit"], true);
  return block({
    type: "pullquote",
...
export function collage(...children: readonly BlockInput[]): BlockValueOf<"collage">;
export function collage(options: CaptionOptions, ...children: readonly BlockInput[]): BlockValueOf<"collage">;
export function collage(first?: CaptionOptions | BlockInput, ...rest: readonly BlockInput[]) {
  const [options = {}, children] = splitOptions<CaptionOptions, BlockInput>(first, rest, "collage()", ["caption", "credit"]);
  return collection("collage", "collage()", options, children);
}
export function slideshow(...children: readonly BlockInput[]): BlockValueOf<"slideshow">;
export function slideshow(options: CaptionOptions, ...children: readonly BlockInput[]): BlockValueOf<"slideshow">;
export function slideshow(first?: CaptionOptions | BlockInput, ...rest: readonly BlockInput[]) {
  const [options = {}, children] = splitOptions<CaptionOptions, BlockInput>(first, rest, "slideshow()", ["caption", "credit"]);
...
export function table(first?: TableOptions | TableRowInput, ...rest: readonly TableRowInput[]) {
  const [options = {}, children] = splitOptions<TableOptions, TableRowInput>(first, rest, "table()", ["bordered", "striped", "caption"]);
  return block({
    type: "table",
...
export function tableCell(first?: TableCellOptions | RichTextInput, ...rest: readonly RichTextInput[]) {
  const [options = {}, children] = splitOptions<TableCellOptions, RichTextInput>(
    first, rest, "tableCell()", ["header", "colspan", "rowspan", "align", "valign"], true,
  );
...
export function details(options: DetailsOptions, ...children: readonly BlockInput[]): BlockValueOf<"details"> {
  return block({
    type: "details",
...
export function map(options: MapOptions, ...children: readonly never[]): BlockValueOf<"map"> {
  ...
  return block({ type: "map", location: options.location, zoom, width, height, ...(richCaption === undefined ? {} : { caption: richCaption }) } as Extract<InputRichBlock, { type: "map" }>);
}
