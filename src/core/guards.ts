import { kindOf, type BlockValue, type ListItemValue, type RichMessageValue, type RichTextValue, type TableCellValue, type TableRowValue } from "../core/values.ts";
...
export function expectRichText(value: unknown): RichTextValue {
  if (kindOf(value) !== "rich-text") throw new TypeError("expectRichText() expects a rich-text element");
...
export function expectTableRow(value: unknown): TableRowValue {
  if (kindOf(value) !== "table-row") throw new TypeError("expectTableRow() only accepts <table-row>");
...
export function expectRichMessage(value: unknown): RichMessageValue {
  if (kindOf(value) !== "rich-message") throw new TypeError("expectRichMessage() expects a <RichMessage> root");
